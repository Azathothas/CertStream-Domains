...
# Runs for 6.0 H (+ 40 Mins) | 400 Mins
 ...
 #Start client
 timeout -k 1m 250m certstream -url "ws://localhost:8888" -domains-only -quiet > "/tmp/certstream_domains.txt"
 #timeout -k 1m 250m certstream -url "wss://certstream.calidog.io" -domains-only -quiet > "/tmp/certstream_domains.txt"
 #timeout -k 1m 250m certstream > "/tmp/certstream_domains.txt"
#Filter & Parse
 #Remove Spaces
